  ...
  class BootstrapTable {
    ...
    initToolbar () {
      ...
      if (this.options.showSearchButton) {
        html.push(Utils.sprintf('<button class="btn btn-default" type="button" name="refresh" id="search_btn" style="border-right:0px;padding-left:7px;padding-right:3px" aria-label="refresh" title="搜索">',
        this.options.formatRefresh()),
        Utils.sprintf('搜索'),'</button>');

        html.push(Utils.sprintf('<button style="border-left:0px;padding-left:3px;padding-right:7px" class="btn btn-default" type="button"  id="search-input" aria-label="refresh" title="搜索">'),
      ...
      if (this.options.showSearchButton) {
        this.$toolbar.find('button[name="refresh"]')
        .off('click').on('click', $.proxy(this.refresh, this));
        this.$toolbar.find('#search-input').off('click').on('click',function() {
            $('#search').toggle();
            $('#search-input span').toggleClass('glyphicon-chevron-down');
        });
      }
